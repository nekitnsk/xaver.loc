...
class AdsStore{
    ...
    function AddUp($db, $post) {
        ...
        $ad = new Ad($post);
        $array_object = $ad->getobjectvars();
        if (count($db->select('SELECT id FROM ?# WHERE id = ? ', 'ad', $post['id'])) == 0) {
            $this->lastId = ($db->query('INSERT INTO ?# (?#) VALUES (?a)', 'ad', array_keys($array_object), array_values($array_object)));
            if ($this->lastId != 0){
    ...
    //получение одного объявления из базы 
    function getLastAd($db){
        
        $ad = $db->selectRow('SELECT id, name, title, price FROM ad WHERE id = ? ', $this->lastId);
        $this->ads['lastAd'] = new AdShort($ad);
