...
if (array_key_exists('id', $_POST)) {                //существует ли ключ id в массиве post 
    setcookie("notice[$_POST[id]]", serialize($_POST));
    $_SESSION['notice'][$_POST['id']] = $_POST;      //если существует то запишем в сессию массив с ключом = id объявления
    ...
    <body>
        <div id="maket">
            ...
            <div id="content">
                ...
                foreach ($_COOKIE['notice'] as $key => $value) {                     //перебираем массив сессии
                    $notice[$key] = unserialize($value);
                    if (true) {                          //работаем  с массивом Notice
                        foreach ($notice as $key => $value) {            //перебираем массив 
                            echo '<tr>'
                            . '<td>' . '<a href = dz7_1.php?change=' . $key . '> ' . $value['title'] . '</a></td>'    //формируем ссылку с названием объявления, в качестве параметра ID объявления 
                            . '<td>' . $value['price'] . ' руб. </td>'        //цена
                            . '<td>' . $value['name'] . '</td>'         //имя клиента 
                            . '<td>' . '<a href = dz7_1.php?del=' . $key . '>Удалить' . '</a></td></tr>'    //здесь делаем ссылку на удаление
                            ;
                        }
                    }
                }
